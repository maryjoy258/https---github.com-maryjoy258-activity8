...
The port number along with a few other options can be changed in [require("@effectful/debugger/config")](https://github.com/awto/effectfuljs/blob/HEAD/../debugger/src/config.ts). It should be loaded and the options should be changed before the runtime is loaded, for example using some specific not-transpiled module.
...
The whole application state is stored in `EDBG.context` object. It stores the current stack, variables, async events queue, etc. It can be changed just in place without any setters/getters. Check interface [State](https://github.com/awto/effectfuljs/blob/HEAD/../debugger/api/interfaces/_state_.state.md) for more details.
...
By default it uses [@effectful/serialization](https://github.com/awto/effectfuljs/blob/HEAD/../serialization) library, but any other can be used instead because all we need to store is `EDBG.context`, `EDBG.journal` and a few global objects (depending on the application).
...
This can be done by specifying [@effectful/serialization](https://github.com/awto/effectfuljs/blob/HEAD/../serialization) descriptor, either in code:
...
Or externally because it is easy to hot-mock functions and state, lie it for example done in "react" special import for storing `MessageChannel/MessagePort` which aren't serializable [@effectful/debugger/react](https://github.com/awto/effectfuljs/blob/HEAD/../debugger/src/react.ts).
...
Check [@effectful/serialization](https://github.com/awto/effectfuljs/blob/HEAD/../serialization) for more details.
...
There is an optional parameter to set serialization options from [@effectful/serialization](https://github.com/awto/effectfuljs/blob/HEAD/../serialization). For `EDBG.capture` it is its first argument which has type [WriteOptions](https://github.com/awto/effectfuljs/blob/HEAD/../serialization/api/interfaces/writeoptions.md), and for `EDBG.restore` it is its second parameter and it has type [ReadOptions](https://github.com/awto/effectfuljs/blob/HEAD/../serialization/api/interfaces/readoptions.md).
...
The time-traveling trace is stored in `EDBG.journal` object, and it can be changed and analyzed without any getter/setter. Its type is described in [interface Journal](https://github.com/awto/effectfuljs/blob/HEAD/../debugger/api/interfaces/_state_.journal.md).
